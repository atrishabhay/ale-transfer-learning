    ...
    def _update(self, dt):
        ...
        while self._sim_time < self._current_time:
            ...
            if not self._sync or act is not None:
                ...
                print("I am here")
                if rand > 0.99:
                    print("And now printing:")
                    conv_frame = self._tm.match_templates(obs, compress=True)
                    cv2.imwrite('./images/{}.png'.format(rand), obs)
                    cv2.imwrite('./images/{}-down.png'.format(rand), conv_frame)
